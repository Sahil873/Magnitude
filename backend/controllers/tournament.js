...
router.post("/tournament", authMiddleware, async (req, res) => {
  try {
    ...
    if (req.user.role !== "Organizer") {
      return res
        .status(403)
        .json({ message: "Unauthorized to create a tournament" });
    }
    ...
    if (!Teams || !Array.isArray(Teams) || Teams.length === 0) {
      return res.status(400).json({
        message: "At least one team is required to create a tournament",
      });
    }
    ...
    for (let teamData of Teams) {
      ...
      }); // Assuming 'name' field exists in Team schema
...
router.put("/tournament/:id", authMiddleware, async (req, res) => {
  try {
    ...
    if (tournament.organizerId.toString() !== req.user.userid) {
      return res
        .status(403)
        .json({ message: "Unauthorized to delete this tournament" });
    }
...
router.delete("/tournament/:id", authMiddleware, async (req, res) => {
  try {
    if (req.user.role !== "Organizer") {
      return res
        .status(403)
        .json({ message: "Unauthorized to delete a tournament" });
    }
    const tournament = await Tournament.findById(req.params.id);
    if (!tournament)
      return res.status(404).json({ message: "Tournament not found" });
  ...
  async (req, res) => {
    try {
      if (req.user.role !== "Organizer") {
        return res
          .status(403)
          .json({ message: "Unauthorized to allocate players" });
      }
      ...
      if (!teams || teams.length === 0) {
        return res
          .status(400)
          .json({ message: "Tournament has no teams to allocate players" });
      }
      ...
      for (let i = 0; i < teams.length; i++) {
        await Team.findByIdAndUpdate(teams[i]._id, {
          $set: { players: teams[i].players },
        });
      }

      res
        .status(201)
        .json({ message: "Players allocated successfully", tournament });
    } catch (error) {
      res
        .status(500)
        .json({ message: "Error allocating players", error: error.message });
    }
  }
);

router.post(
  "/tournament/:id/schedulematches",
  authMiddleware,
  async (req, res) => {
    try {
      if (req.user.role !== "Organizer") {
        return res
          .status(403)
          .json({ message: "Unauthorized to schedule matches" });
      }
      ...
      if (!teams || teams.length < 2) {
        return res.status(400).json({
          message:
            "Tournament must have at least two teams to schedule matches",
        });
      }
      ...
      res
        .status(201)
        .json({ message: "Matches scheduled successfully", schedule });
    } catch (error) {
      res
        .status(500)
        .json({ message: "Error scheduling matches", error: error.message });
    }
  }
);
